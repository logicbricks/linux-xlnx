...
struct adv7511_in_params {
	u32 input_id;
	u32 input_style;
	u32 input_color_depth;
	u32 bit_justification;
	u32 hsync_polarity;
	u32 vsync_polarity;
	u32 clock_delay;
};
...
struct adv7511_out_params {
	...
	u32 output_format;
	u32 output_color_space;
	u32 up_conversion;
	u32 csc_enable;
	u32 csc_scaling_factor;
	u32 hdmi_mode;
};
...
static void adv7511_set_config(struct v4l2_subdev *sd)
{
	struct adv7511_state *state = get_adv7511_state(sd);
...
static void adv7511_get_config(struct i2c_client *client,
			       struct adv7511_state *state)
{
	struct adv7511_config *cfg = &state->cfg;
	struct adv7511_in_params *in = &cfg->in_params;
	struct adv7511_out_params *out = &cfg->out_params;
	struct device_node *parent_dn = client->dev.of_node;
	struct device_node *dn, *coeff_dn;
	int ret;
	bool input = false;
	bool output = false;

	of_property_read_u32(parent_dn, "edid-addr", &state->pdata.i2c_edid);

	dn = of_get_child_by_name(parent_dn, "video-input");
	if (dn) {
		ret = of_property_read_u32(dn, "input-id",
					   &in->input_id);
		if (ret)
			pr_warn("failed get input-id\n");
		ret = of_property_read_u32(dn, "input-style",
					   &in->input_style);
		if (ret)
			pr_warn("failed get input-style\n");
		ret = of_property_read_u32(dn, "input-color-depth",
					   &in->input_color_depth);
		if (ret)
			pr_warn("failed get input-color-depth\n");
		ret = of_property_read_u32(dn, "bit-justification",
					   &in->bit_justification);
		if (ret)
			pr_warn("failed get bit-justification\n");
		ret = of_property_read_u32(dn, "hsync-polarity",
					   &in->hsync_polarity);
		if (ret)
			pr_warn("failed get hsync-polarity\n");
		ret = of_property_read_u32(dn, "vsync-polarity",
					   &in->vsync_polarity);
		if (ret)
			pr_warn("failed get vsync-polarity\n");
		ret = of_property_read_u32(dn, "clock-delay",
					   &in->clock_delay);
		if (ret)
			pr_warn("failed get clock-delay\n");

		of_node_put(dn);

		input = true;
	} else {
		pr_err("failed get video input configuration\n");
	}

	dn = of_get_child_by_name(parent_dn, "video-output");
	if (dn) {
		ret = of_property_read_u32(dn, "hdmi-mode",
					   &out->hdmi_mode);
		if (ret)
			pr_warn("failed get hdmi-mode\n");
		ret = of_property_read_u32(dn, "output-format",
					   &out->output_format);
		if (ret)
			pr_warn("failed get output-format\n");
		ret = of_property_read_u32(dn, "output-color-space",
					   &out->output_color_space);
		if (ret)
			pr_warn("failed get output-color-space\n");
		ret = of_property_read_u32(dn, "up-conversion",
					   &out->up_conversion);
		if (ret)
			pr_warn("failed get up-conversion\n");
		ret = of_property_read_u32(dn, "csc-enable",
					   &out->csc_enable);
		if (ret)
			pr_warn("failed get csc-enable\n");
		if (out->csc_enable) {
			ret = of_property_read_u32(dn, "csc-scaling-factor",
						   &out->csc_scaling_factor);
			if (ret)
				pr_warn("failed get csc-scaling-factor\n");

			coeff_dn = of_find_node_by_name(dn, "csc-coefficients");
			if (coeff_dn) {
				ret = of_property_read_u32(coeff_dn, "a1",
							   &out->csc_coeff.a1);
				ret = of_property_read_u32(coeff_dn, "a2",
							   &out->csc_coeff.a2);
				ret = of_property_read_u32(coeff_dn, "a3",
							   &out->csc_coeff.a3);
				ret = of_property_read_u32(coeff_dn, "a4",
							   &out->csc_coeff.a4);
				ret = of_property_read_u32(coeff_dn, "b1",
							   &out->csc_coeff.b1);
				ret = of_property_read_u32(coeff_dn, "b2",
							   &out->csc_coeff.b2);
				ret = of_property_read_u32(coeff_dn, "b3",
							   &out->csc_coeff.b3);
				ret = of_property_read_u32(coeff_dn, "b4",
							   &out->csc_coeff.b4);
				ret = of_property_read_u32(coeff_dn, "c1",
							   &out->csc_coeff.c1);
				ret = of_property_read_u32(coeff_dn, "c2",
							   &out->csc_coeff.c2);
				ret = of_property_read_u32(coeff_dn, "c3",
							   &out->csc_coeff.c3);
				ret = of_property_read_u32(coeff_dn, "c4",
							   &out->csc_coeff.c4);

				of_node_put(coeff_dn);
			} else {
				pr_info("failed get csc-coefficients\n");
			}
		}
		...
		pr_err("failed get video output configuration\n");
	}
...
static int adv7511_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (client->dev.of_node) {
		adv7511_get_config(client, state);
	} else {
		/* Platform data */
		if (!pdata) {
